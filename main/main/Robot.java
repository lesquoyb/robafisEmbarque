	...
	public void followLine(int color) throws Exception{
		...
		while( ! colorDetected(color) ){
			...
			if(isInWhite(read)){
				...
				//	if(getTacho() / tacho_per_cm > 150){ //pas de virage dans la premi�re partie
						baseSpeed = takeBend();
						initBegin();
						
						if( 	(last_pos == 0 && getGyroValue() % 360 < -150 )
							|| 	(last_pos != 0 && getGyroValue() < -360)	){ //TODO: tester �a proprement, r�flexion faite ce code merde s'il fait un 360 avant le second virage et qu'il refait des virage dans la derni�re ligne
							last_pos = getTacho();
						}
